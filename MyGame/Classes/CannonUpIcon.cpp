...
CannonUpIcon::CannonUpIcon():BaseUpIcon()
{
}


CannonUpIcon::~CannonUpIcon()
{
}

bool CannonUpIcon::init() {//根据选中的图标设置文字描述
	if (!BaseUpIcon::init()) {
		return false;
	}
	getLevel(POWER);
	initIcon();
	kindSprite = Sprite::create("png/kind3.png");
	kindSprite->setScale(0.9f);
	kindSprite->setPosition(0, 0);
	addChild(kindSprite);
	auto listener = EventListenerMouse::create();  //注册鼠标悬停
	listener->onMouseMove = CC_CALLBACK_1(CannonUpIcon::mouseTip, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(listener, forceIcon);
	return true;
}

void CannonUpIcon::setDes(int i) {
	...
	case 3:
		tem = MaxLevel;
		if (tem <= 3) {
			des->setString(__String::createWithFormat("improve cannon towers'\nmaxlevel by 1\nnow maxlevel: %d\nnext level effect:\nlet the maxlevel be %d",
...
void CannonUpIcon::mouseTip(EventMouse* e) {
	...
	if (forceIcon->getBoundingBox().containsPoint(reNode)) {
		briefDes->setString("improve cannon\ntowers' force");
